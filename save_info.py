...
if __name__ == "__main__":
    ...
    try:
        ...
        for i,s in enumerate(sav):
            ...
            print("    * Game Timer (HHH:MM:SS.MMM): %s:%s:%s.%s" % (str(d['gametime'][0]).zfill(3), str(d['gametime'][1]).zfill(2), str(d['gametime'][2]).zfill(2), str(d['gametime'][3]/30.).lstrip('0.')[:3].ljust(3,'0')))
            ...
            print("    * Visible Menus: %s" % ', '.join(d['menu_visible']))
            print("    * Locked Menus: %s" % {True:'None', False:', '.join(d['menu_locked'])}[len(d['menu_locked']) == 0])
            print("    * Countdown (seconds) %d.%s" % (d['countdown'][0], str(d['countdown'][1]).lstrip('0.')))
            print("    * Countdown Timer (HHH:MM:SS.MMM): %s:%s:%s.%s" % (str(d['counttime'][0]).zfill(3), str(d['counttime'][1]).zfill(2), str(d['counttime'][2]).zfill(2), str(d['counttime'][3]/30.).lstrip('0.')[:3].ljust(3,'0')))
            print("    * Encounter Timer: Seed = %d, Offset = %d" % (d['encounter_timer']['seed'], d['encounter_timer']['offset']))
            print("    * Yuffie's Initial Level: %d" % d['yuffie_init_lvl'])
            print("    * Temporary Party Placeholder: %s" % ', '.join([PORTRAIT_TO_NAME[v] for v in d['temp_party'] if v != 255]))
            print("    * Love Points:")
            ...
            for j,items in enumerate(d['field_items']):
                print("      * Location %d: %s" % (j+1, {True:'None',False:', '.join(items)}[len(items) == 0]))
